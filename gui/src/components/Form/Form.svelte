<script lang="ts">
	...
	let message = $state('Ваше сообщение успешно доставлено. Мы свяжемся с вами в ближайшее время.');
	let { theme, question } = $props<string>();
	...
	async function handleSubmit(event) {
		...
		load = true;
		const res = await ky.post('contact', {
			json: {
				theme: theme,
...
<main>
	...
	<div class="formDiv">
		<form id="form" class="form" onsubmit={handleSubmit}>
			...
			<input class="block" type="number" id="phone" name="phone" value="7" /><br /><br />
			<label for="description">{question}</label><br />
			<textarea
				class="block"
				style="overflow-wrap: break-word; max-width: 100%; padding-top: 10px"
				rows="5"
				id="description"
				name="description"
			></textarea><br /><br />
